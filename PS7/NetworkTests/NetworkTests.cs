// Author: Daniel Kopta, Alyssa Johnson, and John Stevens FA21
...
namespace NetworkUtil
{
    ...
    public class NetworkTests
    {
        ...
        [DataTestMethod]
        public void TestSendAndCloseIsItReallyClosedHuh()
        {
            SetupTestConnections(true, out testListener, out testLocalSocketState, out testRemoteSocketState);

            
            //send and close
            Networking.SendAndClose(testLocalSocketState.TheSocket, "help");
            //start the get data process
            Networking.GetData(testRemoteSocketState);
            //wait for the thread to catch up
            NetworkTestHelper.WaitForOrTimeout(() => testRemoteSocketState.GetData().Length > 0, NetworkTestHelper.timeout);
            //compare the data
            Assert.AreEqual("help", testRemoteSocketState.GetData());
            //remove the data
            testRemoteSocketState.RemoveData(0, testRemoteSocketState.GetData().Length);
            //make sure theres really no data left over
            Assert.AreEqual("", testRemoteSocketState.GetData());
            //try sending more data
            Networking.Send(testLocalSocketState.TheSocket, "help");
            //wait for the thread to catch up
            System.Threading.Thread.Sleep(4000);
            //confirm that theres still no data recieved
            Assert.AreEqual("", testRemoteSocketState.GetData());
        }

        [TestMethod]
        public void InvalidHostNameIPAdress()
        {
            NetworkTestHelper.SetupSingleConnectionTest(out testListener, out testLocalSocketState, out testRemoteSocketState);
            bool isCalled = false;
            ...
            Networking.ConnectToServer(saveClientState, "localhostSIKE", 2112);
            NetworkTestHelper.WaitForOrTimeout(() => isCalled, NetworkTestHelper.timeout);
